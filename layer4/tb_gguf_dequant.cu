    ...
    const float * __restrict__ x, float * __restrict__ out)
{
    ...
    for (int sbi = threadIdx.x; sbi < nsb; sbi += blockDim.x) {
        ...
        for (int j=0;j<4;j++) {
            ...
            mn8[j+4] = dm * (float)((sc[j+4]>>6)|((sc[j+12]&0x0F)<<2));
        }
        int bk = sbi*QK_K;
        for (int i=0;i<128;i++) {
            uint8_t b = qs[i];
    ...
    if (threadIdx.x == 0) out[row] = block_reduce(acc, smem);
    else block_reduce(acc, smem);
}
                           ...
                           const float *x_host, float *out_host)
{
    ...
    switch (qtype) {
    case 12: k_q4k_matvec  <<<grd,blk,0,st>>>(Wd,M,K,g_d_x,g_d_out[si]); break;
    case 11: k_q3k_matvec  <<<grd,blk,0,st>>>(Wd,M,K,g_d_x,g_d_out[si]); break;
    case 14: k_q6k_matvec  <<<grd,blk,0,st>>>(Wd,M,K,g_d_x,g_d_out[si]); break;
    case  2: k_q4_0_matvec <<<grd,blk,0,st>>>(Wd,M,K,g_d_x,g_d_out[si]); break;
    case  8: k_q8_0_matvec <<<grd,blk,0,st>>>(Wd,M,K,g_d_x,g_d_out[si]); break;
    case 15: k_q8k_matvec  <<<grd,blk,0,st>>>(Wd,M,K,g_d_x,g_d_out[si]); break;
    case 30: k_bf16_matvec <<<grd,blk,0,st>>>((const uint16_t*)Wd,M,K,g_d_x,g_d_out[si]); break;
    case  0: k_f32_matvec  <<<grd,blk,0,st>>>((const float*)Wd,M,K,g_d_x,g_d_out[si]); break;
    default: return 0;
    }
    if (cudaGetLastError()!=cudaSuccess) return 0;
    if (cudaMemcpyAsync(g_h_pin[si],g_d_out[si],(size_t)M*sizeof(float),
                        cudaMemcpyDeviceToHost,st)!=cudaSuccess) return 0;
