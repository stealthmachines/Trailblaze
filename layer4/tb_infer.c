...
static int tb_layer_forward(
    ...
    if (m->n_experts > 0) {
        ...
        for (int ei = 0; ei < k; ei++) {
            ...
            TNSR("blk.%d.ffn_gate_exps.weight", layer_idx);
            const TB_GGUFTensorInfo *eg_t = tb_gguf_find_tensor(m, tname);
            /* Note: in packed expert format, all experts are in one tensor */
...
int tb_infer_decode(TB_InferCtx *ctx, int last_token, int pos, int session_id) {
    ...
    if (emb_t && m->weights_data && last_token >= 0 && last_token < m->vocab_size) {
        /* Each embedding row: H elements in BF16 or F32 */
        if (emb_t->qtype == 30) {
            ...
            /* Quantized embedding: dequantise row [last_token] via tb_gguf_dequant_row */
            int bw, bb;
            switch (emb_t->qtype) {
                case 8:  bw=32;  bb=34;  break;  /* Q8_0 */
                case 2:  bw=32;  bb=18;  break;  /* Q4_0 */
                case 12: bw=256; bb=144; break;  /* Q4_K */
                ...
                default: bw=1;   bb=4;   break;  /* F32 fallback */
            }
        ...
        /* No embedding tensor: synthetic */
        for (int i=0;i<H;i++) x[i]=0.01f*sinf((float)(i+last_token));
    }

    /* Per-layer KV cache */
    TB_KVCache *kv = NULL;
    if (m->n_kv_heads > 0 && m->head_dim > 0) {
        kv = tb_kvcache_alloc(NL, m->n_kv_heads, m->head_dim,
                               m->max_seq_len, session_id, ctx->lattice->epoch);
    }
    ...
    /* Final norm */
    float *norm_out = (float*)malloc(H * sizeof(float));
    ...
    if (!lm_t) lm_t = tb_gguf_find_tensor(m, "token_embd.weight"); /* tied */
    if (lm_t && m->weights_data) {
        /* Universal: handles F32/BF16/F16 and all quantized types */
        tb_gguf_dequant_matvec(tb_gguf_tensor_data(m, lm_t), lm_t->qtype,
                               m->vocab_size, H, norm_out, logits);
    } else {
        /* Synthetic: argmax returns a reasonable token */
    ...
    free(x); free(y); free(norm_out); free(ones); free(logits);
    tb_kvcache_free(kv);
    return next_token;
}
...
static void* tb_handle_conn(void *arg) {
    ...
    char buf[8192] = {0};
    int n = (int)TB_RECV(fd, buf, sizeof(buf)-1);
    if (n <= 0) { TB_CLOSESOCK(fd); return NULL; }
    buf[n] = '\0';
    ...
    if (strcmp(method, "OPTIONS") == 0) {
        ...
        tb_http_response(fd, 200, "application/json", body);

    } else if (strcmp(path, "/api/generate") == 0 && strcmp(method,"POST")==0) {
        /* Find JSON body after \r\n\r\n */
        const char *body_start = strstr(buf, "\r\n\r\n");
        if (!body_start) { tb_http_response(fd,400,"application/json","{\"error\":\"no body\"}"); TB_CLOSESOCK(fd); return NULL; }
        body_start += 4;

        /* Extract "prompt" field */
        const char *pstart = strstr(body_start, "\"prompt\"");
        char prompt[2048] = "Hello";
        if (pstart) {
            pstart = strchr(pstart, ':');
            if (pstart) { pstart++; while (*pstart==' '||*pstart=='"') pstart++;
                int pl=0; while (pstart[pl] && pstart[pl]!='"' && pl<2047) pl++;
                memcpy(prompt, pstart, pl); prompt[pl]='\0'; }
        }

        /* Tokenize prompt */
        int token_ids[512]; int n_tok = 0;
        TB_Tokenizer *tok = ctx->model ? ctx->model->tokenizer : NULL;
        if (tok) {
            n_tok = tb_tokenizer_encode(tok, prompt, 1, token_ids, 512);
        }
        if (n_tok <= 0) {
            /* Byte-level fallback */
            for (int i=0;i<(int)strlen(prompt)&&n_tok<512;i++)
        ...
        /* Generate */
        int out_ids[512]; int n_gen=0;
        pthread_mutex_lock(&ctx->generate_lock);
        n_gen = tb_infer_generate(ctx, token_ids, n_tok,
                                   out_ids, 256, 0, NULL, NULL);
        ...
        if (tok && n_gen > 0) {
            ...
            if (decoded) {
                snprintf(resp_text, sizeof(resp_text), "%s", decoded);
                free(decoded);
            }
        }
        ...
        snprintf(resp, sizeof(resp),
            ...
            "\"response\":\"%.*s\"}",
            cc->cfg->model_name, n_gen,
            (long long)(ctx->decode_ms * 1e6),
            (int)(sizeof(resp)-200), resp_text);
        tb_http_response(fd, 200, "application/json", resp);

    } else if (strcmp(path, "/api/chat") == 0 && strcmp(method,"POST")==0) {
        /* Minimal chat: extract last user message content */
        const char *body_start = strstr(buf, "\r\n\r\n");
        char content[1024] = "Hello";
        if (body_start) {
            const char *cont = strstr(body_start, "\"content\"");
            if (cont) {
                cont = strchr(cont, ':');
                if (cont) { cont++; while(*cont==' '||*cont=='"') cont++;
                    int cl=0; while(cont[cl]&&cont[cl]!='"'&&cl<1023) cl++;
                    memcpy(content, cont, cl); content[cl]='\0'; }
            }
        }
        /* Tokenize content */
        int token_ids[512]; int n_tok = 0;
        TB_Tokenizer *tok = ctx->model ? ctx->model->tokenizer : NULL;
        if (tok) {
            n_tok = tb_tokenizer_encode(tok, content, 1, token_ids, 512);
        }
        if (n_tok <= 0) {
            for (int i=0;i<(int)strlen(content)&&n_tok<512;i++)
                token_ids[n_tok++]=((unsigned char)content[i])
                                   %(ctx->model->vocab_size>0?ctx->model->vocab_size:32000);
        }
        if (n_tok <= 0) { token_ids[0]=1; n_tok=1; }
        ...
        snprintf(resp, sizeof(resp),
            ...
            "\"eval_count\":%d}",
            cc->cfg->model_name, (int)(sizeof(resp)-200), resp_text, n_gen);
        tb_http_response(fd, 200, "application/json", resp);

    } else if (cc->cfg->ext_handler) {
        const char *body_start = strstr(buf, "\r\n\r\n");
        const char *req_body   = body_start ? body_start + 4 : "";
        cc->cfg->ext_handler(fd, path, method, req_body, cc->cfg->ext_ctx);
    } else {
