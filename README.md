# TRAILBLAZE v0.6 — Qwen3 Inference Runtime
...
```bash
# 1. Clone and build (Linux / WSL — gcc required):
git clone https://github.com/stealthmachines/Trailblaze.git -b v0.6
cd Trailblaze
bash scripts/build.sh          # produces bin/tb_infer

# 2. Run against the smallest available Qwen3 model:
bash scripts/run.sh --4b       # auto-locates Qwen3.5-4B GGUF

# 3. Or point at any GGUF directly:
./bin/tb_infer --model /path/to/model.gguf --port 11434
...
**Requirements:** `gcc ≥ 11`, `libm`, `libpthread` (standard on Ubuntu/Debian).
Optional: CUDA toolkit ≥ 11.4 for GPU acceleration (pass `-DTB_CUDA` or let `build.sh` detect).
...
bash scripts/run.sh --no-hdgl                    # standard top-k routing only
bash scripts/run.sh --model /path/to/model.gguf --port 11434
./bin/tb_infer --benchmark                       # synthetic self-test
...
```bash
# Start trailblaze on default port:
bash scripts/run.sh --4b

# Point Open WebUI at it (Ollama mode, no changes needed):
