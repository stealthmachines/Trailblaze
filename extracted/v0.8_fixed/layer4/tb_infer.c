...
int tb_layer_forward(
    ...
    if (!is_local) {
        ...
        /* RoPE */
        for (int h=0;h<NH;h++) tb_rope_apply(q+h*HD, NULL,   HD, pos, m->rope_base);
        for (int h=0;h<NK;h++) tb_rope_apply(NULL,   k+h*HD, HD, pos, m->rope_base);
        ...
        /* conv_t GGUF shape: [K=4, M=8192] stored row-major [M rows × K cols]
